...
def upload_to_cloudinary(api_secret,filename,resource_type = "image", extra_field = {}): 
    api_key = "871173851884581"
    cloud_name = "daxxlcdlw"
    body = {
        "api_key": api_key, 
    }
...
def lambda_handler(event,context):
    ...
    cloudinary_secret = str(params["/the-last-show/cloudinary-secret-key"])
    ...
    res = upload_to_cloudinary(api_secret=cloudinary_secret,filename=file_name, extra_field={"eager": "e_art:zorro,e_grayscale,w_150,h_150,c_fill,g_face,r_max"}) 
    ...
    response = dynamodb.Table("obituaries-30153653").put_item(
        Item={
